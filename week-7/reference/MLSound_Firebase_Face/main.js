...
async function askForSound(p_prompt) {
    ...
    in_front_of_you.position.set(0, 0, -distanceFromCenter); // if it's a child to sth, this means setting the relative position to its parent: the camera, which is in (0,0,0)
    in_front_of_you.getWorldPosition(posInWorld); // get the absolute position
    let location = { x: posInWorld.x, y: posInWorld.y, z: posInWorld.z * 2 };
    console.log("in_front_pos", in_front_of_you.position);
